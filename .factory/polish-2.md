...
|---|---|---|
| F-2-1 | Added a compact sample result and **Move Yes later** action directly below the persistent demo banner; moved editable table details below the proofing panels. | Mobile test `fits the core workflow on a 390px viewport`; local `/demo` screenshot/check. |
| F-2-2 | Removed the unprovable USD 19 and one-time-price copy and its circular claim. The checkout link remains without an unsupported price promise. | `@claim:sociobot-billing-path`; no price claim remains in app, README, Privacy, or Terms. |
| F-2-3 | Removed merchant-of-record and refund claims rather than infer legal facts from checkout copy. | Copy audit; no merchant claim remains. |
...
| F-2-6 | Removed the untestable card-details and hosted-checkout boundary statement. | Privacy copy audit; `@claim:sociobot-billing-path` covers the actual registered path. |
| F-2-7 | Removed the unsupported refund-handling statement. | Terms copy audit. |
...
| F-2-11 | Replaced the first-screen browser fact with the immediately useful price fact “Core checking and exports are free.” | Mobile landing inspection; `@claim:free-core`. |
