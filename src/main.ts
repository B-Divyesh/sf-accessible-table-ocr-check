...
function legalPage(kind: 'privacy' | 'terms') {
  ...
  app.innerHTML = shell(`<article class="legal-page"><p class="eyebrow">Plain-language policy · 30 August 2026</p><h1>${privacy ? 'How we handle your documents' : 'Terms of use'}</h1>${privacy ? `
    ...
    <h2>One-time Desk license</h2><p>The optional $19 Desk license adds local saved versions on supported devices. Core checking and HTML, CSV, JSON, and issue-report exports remain free. Sociobot/Dodo is the merchant of record and handles checkout and refunds. A refunded or revoked license stops paid features.</p>
...
function workbench() {
  ...
  app.innerHTML = shell(`<div class="workbench-heading"><div><p class="eyebrow">${demoMode ? 'Sample table check · isolated demo' : 'Open table check · saved locally'}</p><h1>Review reading order</h1></div><div class="proof-meta"><label>Table check name<input id="project-name" value="${escapeHtml(project.name)}"></label><label>Source reference<input id="source-page" value="${escapeHtml(project.sourcePage)}"></label></div><button class="button quiet danger-button" type="button" data-action="new">Clear table check</button></div>
